...
// or from the last difficulty change if 'lookup' is nonpositive.
// If 'height' is nonnegative, compute the estimate at the time when a given block was found.
Value GetNetworkHashPSRecent(int lookup, int height) {
    CBlockIndex *pb = pindexBest;
...
Value getnetworkhashpsrecent(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            "getnetworkhashps [blocks] [height]\n"
    ...
    return GetNetworkHashPSRecent(params.size() > 0 ? params[0].get_int() : 10, params.size() > 1 ? params[1].get_int() : -1);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("networkhashpsrecent", getnetworkhashpsrecent(params, false)));
