...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // GreenCoin: starting difficulty is 1 / 2^12
...
{
    // GreenCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
 ...
 // Only change once per interval
    ...
    // GreenCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, int nHeight, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // GreenCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // GreenCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
   	pchMessageStart[0] = 0xfc;
        ...
        hashGenesisBlock = uint256("0x39a5aa8e2fbd942611a86c5a47decddf3e20e0475ce9e2353e6e2c0c0dd82691");
    }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Greencoin: increase each by adding 2 to bitcoin's value.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
	...
	int64 greencoinreward = GetBlockValue(pindexPrev->nHeight+1,0);
	int64 rewardAmount = greencoinreward / 2;
	pblock->vtx[0].vout[0].nValue = rewardAmount;

        pblock->vtx[0].vout[1].nValue = greencoinreward - rewardAmount + nFees;
...
void static GreencoinMiner(CWallet *pwallet)
{
    printf("GreenCoinMiner:\n");
    ...
    try { loop {
        ...
        printf("Running GreenCoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GreencoinMiner, pwallet));
}
